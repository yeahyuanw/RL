...
bool L1_flag=1;
string version = "bern";
int entity_space = 100;
int relation_space = 100;
double margin_test = 1;
double rate_test = 0.001;
string resultfile = "result_001_1_100_L1";
int hit_n = 10;
int rerank_num = 500;
...
private:
    ...
    void entity_prediction()
    {
        ...
        vector<pair<int,int> >  l_raw_rank,r_raw_rank,l_filter_rank,r_filter_rank;
        l_raw_rank.resize(fb_h_size/2);
        r_raw_rank.resize(fb_h_size/2);
        l_filter_rank.resize(fb_h_size/2);
        r_filter_rank.resize(fb_h_size/2);
        ...
        for(int testId = 0; testId<fb_h_size/2; testId++)
        {
            ...
            #pragma omp parallel for num_threads(6) schedule(dynamic,10)
            ...
            l_raw_rank[testId] = make_pair(relation_type[rel],l_raw_num);
            r_raw_rank[testId] = make_pair(relation_type[rel],r_raw_num);
            l_filter_rank[testId] = make_pair(relation_type[rel],l_filter_num);
            r_filter_rank[testId] =make_pair(relation_type[rel],r_filter_num);
            /*if(testId%10 == 0)
                cout<<testId<<"--"<<fb_h_size/2<<endl;*/
        } //for(int testId = 0; testId<fb_h_size; testId++)

        FILE*file_l_raw_rank = fopen((resultfile+"/l_raw_rank.txt").c_str(),"w");
        FILE*file_r_raw_rank = fopen((resultfile+"/r_raw_rank.txt").c_str(),"w");
        FILE*file_l_filter_rank = fopen((resultfile+"/l_filter_rank.txt").c_str(),"w");
        FILE*file_r_filter_rank = fopen((resultfile+"/r_filter_rank.txt").c_str(),"w");
        for(int i = 0; i<fb_h_size/2; i++)
        {
            fprintf(file_l_raw_rank,"%d\t%d\t%d\n",i,l_raw_rank[i].first,l_raw_rank[i].second+1);
            fprintf(file_r_raw_rank,"%d\t%d\t%d\n",i,r_raw_rank[i].first,r_raw_rank[i].second+1);
            fprintf(file_l_filter_rank,"%d\t%d\t%d\n",i,l_filter_rank[i].first,l_filter_rank[i].second+1);
            fprintf(file_r_filter_rank,"%d\t%d\t%d\n",i,r_filter_rank[i].first,r_filter_rank[i].second+1);
        }
        fclose(file_l_raw_rank);
        fclose(file_r_raw_rank);
        fclose(file_l_filter_rank);
        fclose(file_r_filter_rank);
    ...
    void relation_predictive()
    {
        ...
        vector<pair<int,int> > rel_raw_rank,rel_filter_rank;
        rel_raw_rank.resize(fb_h_size/2);
        rel_filter_rank.resize(fb_h_size/2);

        cout<<"start relation prediction!"<<endl;

        #pragma omp parallel for num_threads(220) schedule(dynamic,10)
        for(int testId = 0; testId<fb_h_size/2; testId++)
        {
            int e1 = fb_h[testId*2];
            int e2 = fb_l[testId*2];
            ...
            rel_raw_rank[testId] = make_pair(relation_type[rel],rel_raw_num);
            rel_filter_rank[testId] = make_pair(relation_type[rel],rel_filter_num);
            /*if(testId%10 == 0)
                cout<<testId<<"--"<<fb_h_size/2<<endl;*/
        }//for(int testId = 0;testId<fb_h_size/2;testId++)
        FILE*file_rel_raw  = fopen((resultfile+"/rel_raw_rank.txt").c_str(),"w");
        FILE*file_rel_filter = fopen((resultfile+"/rel_filter_rank.txt").c_str(),"w");

        for(int i = 0; i<fb_h_size/2; i++)
        {
            //testId,type,rank
            fprintf(file_rel_raw,"%d\t%d\t%d\n",i,rel_raw_rank[i].first,rel_raw_rank[i].second+1);
            fprintf(file_rel_filter,"%d\t%d\t%d\n",i,rel_filter_rank[i].first,rel_filter_rank[i].second+1);
        }
        fclose(file_rel_raw);
        fclose(file_rel_filter);
...
void prepare()
{
    ...
    FILE* f_path = fopen("experiment_data/path2.txt","r");
    ...
    FILE*file_train = fopen("experiment_data/train.txt","r");
    ...
    FILE*file_valid = fopen("experiment_data/valid.txt","r");
    ...
    FILE* f_confidence = fopen("experiment_data/confidence.txt","r");
