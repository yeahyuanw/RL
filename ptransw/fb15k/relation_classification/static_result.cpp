...
int main(int argc,char**argv)
{
    ...
    FILE* f_rel_raw = fopen(("../"+file_path+"/rel_raw_rank.txt").c_str(),"r");
    FILE* f_rel_filter = fopen(("../"+file_path+"/rel_filter_rank.txt").c_str(),"r");

    while (fscanf(f_rel_raw,"%d",&testId)==1)
    {
    int t = 0,res = 0;
    fscanf(f_rel_raw,"%d",&t);
    fscanf(f_rel_raw,"%d",&res);
    rel_raw.push_back(res);
    }

    while (fscanf(f_rel_filter,"%d",&testId)==1)
    {
    int t = 0,res = 0;
    fscanf(f_rel_filter,"%d",&t);
    fscanf(f_rel_filter,"%d",&res);
    rel_filter.push_back(res);
    }
    ...
    double l_raw_sum = 0,l_filter_sum = 0,r_raw_sum = 0,r_filter_sum = 0;
    double l_raw_num = 0,l_filter_num = 0,r_raw_num = 0,r_filter_num = 0;

    double rel_raw_sum = 0,rel_filter_sum = 0;
    double rel_raw_num = 0,rel_filter_num = 0;
    ...
    for(int id = 0;id<test_num;id++)
    {
        ...
        if(fb_path[ttt].size()>0)
        {
            ...
            if(l_raw[id] <= entity_hit_n)
            {
                l_raw_num++;
            }
            if(l_filter[id] <= entity_hit_n)
            {
                l_filter_num++;
            }
            if(r_raw[id] <= entity_hit_n)
            {
                r_raw_num++;
            }
            if(r_filter[id] <= entity_hit_n)
            {
                r_filter_num++;
            }

            rel_raw_sum +=rel_raw[id];
            rel_filter_sum +=rel_filter[id];

            if(rel_raw[id] <= rel_hit_n)
            {
                rel_raw_num++;
            }
            if(rel_filter[id] <= rel_hit_n)
            {
                rel_filter_num++;
            }
        }
    ...
    cout<<endl;
    cout<<"l_raw_num = "<<l_raw_num/path_triplet_num<<"  l_filter_num = "<<l_filter_num/path_triplet_num<<endl;
    cout<<"r_raw_num = "<<r_raw_num/path_triplet_num<<"  r_filter_num = "<<r_filter_num/path_triplet_num<<endl;
    cout<<"hit raw : "<<(l_raw_num+r_raw_num)/(2*path_triplet_num)<<"  hit  filter : "<<(l_filter_num+r_filter_num)/(2*path_triplet_num)<<endl;
     cout<<"--------relation prediction------"<<endl;
    cout<<"rel_raw_sum = "<<rel_raw_sum/path_triplet_num<<"  rel_filter_sum = "<<rel_filter_sum/path_triplet_num<<endl;
    cout<<endl;
     cout<<"rel_raw_num = "<<rel_raw_num/path_triplet_num<<"  rel_filter_num = "<<rel_filter_num/path_triplet_num<<endl;
}
