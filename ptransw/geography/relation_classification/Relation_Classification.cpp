...
const char* file_triplet = "../experiment_data/triplet.txt";

char buf[100000],buf1[100000];
int relation_num,entity_num,test_triplet_num,train_triplet_num,triplet_num;
...
int main(int argc,char**argv)
{
	...
	fclose(f1);
	fclose(f2);
    FILE* f_kb = fopen(file_triplet,"r");
	    ...
	    left_iter++,right_iter++)
	{
		...
		if (left_iter->first == right_iter->first)
		{
			...
			fprintf(relation_weight,"%d\t%f\t%f\t%lf\t%d\n",left_iter->first,t1,t2,ww,type);
		}
		else
		{
	...
	FILE* f_kb_text = fopen(file_triplet,"r");
