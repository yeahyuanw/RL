...
//高斯分布，若随机变量x服从一个数学期望为miu，方差为sigma^2的分布
...
public:
    ...
    void run(int n_in,double rate_in,double margin_in,int method_in)
    {
        ...
        FILE* f1 = fopen(("../transE/"+result_path+"/entity2vec.bern").c_str(),"r");
        ...
        FILE* f2 = fopen(("../transE/"+result_path+"/relation2vec.bern").c_str(),"r");
        for (int i=0; i<relation_num; i++)
        {
            for (int ii=0; ii<m; ii++)
...
private:
    ...
    void bfgs()
    {
        ...
        for (int epoch=0; epoch<nepoch; epoch++)
        {
            ...
            if((epoch+1)%50 == 0)
            {
                FILE* f1 = fopen((result_path+"/relation2vec."+version).c_str(),"w");
                FILE* f2 = fopen((result_path+"/entity2vec."+version).c_str(),"w");
                FILE* f3 = fopen((result_path+"/A."+version).c_str(),"w");
                for (int i=0; i<relation_num; i++)
                {
                    for (int ii=0; ii<m; ii++)
...
void prepare()
{
    ...
    FILE* f_kb = fopen("../experiment_data/train.txt","r");
