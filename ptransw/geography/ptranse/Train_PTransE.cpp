...
private:
    ...
    void bfgs()
    {
        ...
        for (int eval=0; eval<neval; eval++)
        {
            ...
            {
                #pragma omp parallel for schedule(dynamic,10) reduction(+:res) num_threads(100)
    ...
    {
		vector<double>  temp_entity_e1_a = entity_tmp[e1_a];
        vector<double>  temp_entity_e2_a = entity_tmp[e2_a];
        vector<double>  temp_relation_a = relation_tmp[rel_a];

		vector<double>  temp_entity_e1_b = entity_tmp[e1_b];
        vector<double>  temp_entity_e2_b = entity_tmp[e2_b];
        vector<double>  temp_relation_b = relation_tmp[rel_b];
        ...
        for (int ii=0; ii<n; ii++)
        {
            ...
            if (L1_flag)
                if (x>0)
                    ...
                    x=-1;
            temp_relation_a[ii]-=-1*rate*x;
            temp_entity_e1_a[ii]-=-1*rate*x;
            temp_entity_e2_a[ii]+=-1*rate*x;
            ...
            if (L1_flag)
                if (x>0)
                    ...
                    x=-1;
            temp_relation_b[ii]-=rate*x;
            temp_entity_e1_b[ii]-=rate*x;
            temp_entity_e2_b[ii]+=rate*x;
        }

		norm(temp_relation_a);
        norm(temp_entity_e1_a);
        norm(temp_entity_e2_a);

		norm(temp_relation_b);
        norm(temp_entity_e1_b);
        norm(temp_entity_e2_b);
    }
